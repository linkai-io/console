...
export default {
  ...
  methods: {
    ...
    async getInputs() {
      try {
        this.top_ten_addresses = [];
        let params = {
          start: 0,
          discovered_by: 'input_list',
          limit: 10
        };
        let response = await API.get('/address/group/' + this.group.group_id, {
          params: params
        });
        if (
        ...
        let topTen = response.data.addresses.map(v => {
          if (v.host_address === '') {
            return v.ip_address;
          }
          return v.host_address;
        });
        this.top_ten_addresses.push(...topTen);
      } catch (e) {
        console.log(e);
      }
    },
